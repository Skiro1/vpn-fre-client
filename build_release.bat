...
chcp 65001 >nul
echo === VPN-Client RELEASE BUILD ===
echo.

REM ---------------------------------------------------------------
REM Step 1: ensure venv + latest external binaries exist.
REM Always calls tools\setup.bat so that the latest published versions
REM of zapret, zapret2, lua scripts, lists and proxy executables are
REM pulled in. setup.bat is fast on subsequent runs (idempotent).
REM ---------------------------------------------------------------
...
echo.
echo Build complete. EXE: dist\VPN-Client.exe
pause
